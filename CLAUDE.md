...
- **Historical bundle**: `StatScout/Data/players-historical.plist` (33,023 rows: seasons 2000–2024 plus the season-0 career rollup, verified 2026-08-07) regenerated via `source ~/.football_credentials && python3 scripts/export_historical.py` (fetches every season below the current one, then runs the swift plist converter). Regenerate before shipping a build into a new season so the newly-historical year is in the bundle.
